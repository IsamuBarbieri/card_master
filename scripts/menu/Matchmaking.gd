...
func _ready() -> void:
	...
	status_label = _make_label(Vector2(0, 200), 40, Color.BLACK)
	power_label = _make_label(Vector2(0, 262), 26, Color(0.20, 0.18, 0.14))
	elapsed_label = _make_label(Vector2(0, 300), 22, Color(0.35, 0.30, 0.25))

	var cancel := FixedSizeButton.new()
	UIButtonStyle.apply(cancel)
	cancel.text = StringTable.get_string(StringTable.ID_CANCEL)
	cancel.position = Vector2(42, 463)
	...
	cancel.add_theme_color_override("font_shadow_color", Color(0, 0, 0, 0.5))
